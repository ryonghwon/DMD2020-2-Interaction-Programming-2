...
var orders = { name: ['장우진'], coffee: ['아메리카노', '에스프레소', '카페라떼'] };
...
var woo = {
    ...
    major: '디지털미디어디자인',
    grade: 2,
    like: ['전자기기', '잠', '치킨', '아이스크림'],
};
var likes = '';
for (var i = 0; i < woo.like.length; i++) {
    if (i < woo.like.length - 1) {
        likes += woo.like[i] + ', ';
    } else {
        likes += woo.like[i];
    }
...
function introduce() {
    console.log(`안녕하세요 제 이름은 ${woo.name}이고 나이는 ${woo.age}, 사는 곳은 ${woo.location}입니다.`);
    console.log(`학교는 ${woo.school}에 다니고 있으며 과는 ${woo.major}, 학년은 ${woo.grade}학년입니다.`);
    console.log(`제가 좋아하는 것들은 ${likes}입니다.`);
}
introduce();
