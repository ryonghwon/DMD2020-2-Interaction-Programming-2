...
selectors.dom.push('main', 'footer'); // 배열에 원소 추가
// console.log(selectors); // {number: 2, string: " ", dom: Array(3)}
delete selectors.string; // string 삭제
...
*/
// Answer 4.
var orders = { name : ['이유정'], coffee : ['아메리카노', '에소프레스', '카페라떼'] };
for(var i = 0; i < orders.coffee.length; i++){
    console.log(orders.name + '님, 주문하신 ' + orders.coffee[i] + ' 1잔 나왔습니다.');
    // 이유정님, 주문하신 아메리카노 1잔 나왔습니다.
    // 이유정님, 주문하신 에소프레스 1잔 나왔습니다.
    // 이유정님, 주문하신 카페라떼 1잔 나왔습니다.
}
...
var profile = {
    name : '이유정',
    age : 22,
    univ : ['계원예술대학교', '디지털미디어디자인과', '프로그래밍반'],
}
profile.hello = '안녕하세요.';

var result = '';
for(var i = 0; i < profile.univ.length; i++) {
    result = profile.univ.join(' ');

}
console.log( profile.hello + result + '에 재학중인 ' + profile.age + '살 ' + profile.name + '입니다.');

function coffee(brand, menu){
    return '저는 ' + brand +'의 ' + menu + '를(을) 가장 좋아합니다.'
}
var like = coffee('스타벅스', '바닐라라떼');
console.log(like);

// 안녕하세요. 계원예술대학교 디지털미디어디자인과 프로그래밍반에 재학중인 22살 이유정입니다.
// 저는 스타벅스의 바닐라 라떼를(을) 가장 좋아합니다.
