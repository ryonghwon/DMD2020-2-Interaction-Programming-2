...
*/
// Answer 4.
var orders = {name: '박지수', coffee: ['아메리카노','에스프레소','카페라떼']};
...
var result = '';
for(var i = 0;i < info.like.length;i++){
    result = info.like.join();
}
console.log('안녕하세요. 제 이름은 ' + info.name + '이고 나이는 ' + info.age + '살 입니다. 제가 좋아하는 것들은 ' + info.like + '입니다.');
function movie(title, actor){
    return '요즘 빠진 영화는 ' + title + ' 이고 좋아하는 영화배우는 ' + actor + '입니다.'; 
}
var txt = movie('작은 아씨들','티모시 샬라메');
console.log(txt);
