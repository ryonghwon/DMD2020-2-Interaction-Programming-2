...
(function($){
    $(document).ready(function() {
        ...
        function setting(){
            ...
            _isReverse = false;
            // 전체 이미지 영역 / 컬럼 개수
            _itemW = _imgW / _col; // 이미지에서 아이템 1개의 가로 영역
            _itemH = _imgH / _row; // 이미지에서 아이템 1개의 세로 영역
        }
        ...
        function progressFrame(){
            ...
            if(!_isReverse){ // false(정방향) : 프레임 증가
                _cuFrame++;
            }else{ // true(역방향) : 프레임 감소
                _cuFrame--;
            }

            // 첫 프레임은 0. : _cuFrame 값이 0보다 작아질 수 없다.
            if(_cuFrame <= 0){
                _cuFrame = 0;
                
                // 역방향 설정 후 0 이하가 될 경우 정지.
        ...
        function updateFrame(){
            var posX = 0; // 스프라이트 이미제어서의 현재 프레임 X 좌표.
            var posY = 0; // 스프라이트 이미제어서의 현재 프레임 Y 좌표.
