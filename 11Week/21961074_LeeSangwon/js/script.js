...
//         function setting(){
//             // 시퀀스의 반전 여부 (반대로 돌아가고있는지 아닌지 상태 체크)
//             _isReverse = false;

//             // 이미지에서 아이템 1개의 가로 길이 영역
//             _itemW = _imgW / _col; 
//             // 이미지에서 아이템 1개의 세로 높이 영역
//             _itemH = _imgH / _row;
//         }
...
//             var posX = 0, // X 좌표 - 스프라이트 이미지에서의 현재 프레임 x 좌표
//                 posY = 0; // Y 좌표 - 스프라이트 이미지에서의 현재 프레임 y 좌표.

//             //  나머지를 구하는 공식으로 x 의 셀 위치를 찾아온다.
//             // posX = _cuFrame % _row;
//             // _col - Column 몇 단으로 해당되는지 - 7단으로 이미지 구성

//             //  나누기 공식으로 y 의 셀 위치를 찾아온다.
//             //  소수점 내림을 처리를 해준다.
//             // posY = Math.floor(_cuFrame / _col);
//             // _row - Row 몇 줄에 해당되는지 - 7줄로 이미지 구성
...
//             posX = _cuFrame % _col * _itemW; // 이미지에서의 실제 x 좌표 
//             posY = Math.floor(_cuFrame / _row) * _itemH * -1; // 이미지에서의 실제 y 좌표
//             // console.log(_cuFrame, posX, posY);
...
(function($){
    $(document).ready(function() {
        ...
        function setting(){
            // 시퀀스의 반전 상태 여부
            _isReverse = false;
            _itemW = _imgW / _col; // 이미지에서 아이템 영역 1개의 가로 영역
        ...
        function progressFrame(){
            ...
            // 찻 프레임은 0. - -cuFrame 값이 0 보다 작아질 수 없다.
            if(_cuFrame <= 0){
                _cuFrame = 0;
                if(_isReverse) stopFrame(); // 역방향으로 설정 후 0이하가 될 경우 정지.
            }
        ...
        function updateFrame(){
            ...
            posX = _cuFrame % _row * _itemW * -1; // 이미지에서의 실제 X 위치 좌표.
